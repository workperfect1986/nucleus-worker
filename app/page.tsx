    ...
    <section className="workspace">
      ...
      <div className="content">
        ...
        <section className="orders-section" data-tab={tab}>
          ...
          {tab === "active" && <div className="table-wrap active-orders-table"><table><thead><tr><th><button className="table-sort-button" onClick={() => toggleSort("id")}>ID OS <span>{sortIndicator("id")}</span></button></th><th><button className="table-sort-button" onClick={() => toggleSort("id")}>V <span>{sortIndicator("id")}</span></button></th><th>Pedido</th><th><button className="table-sort-button" onClick={() => toggleSort("client")}>Nome <span>{sortIndicator("client")}</span></button></th><th><button className="table-sort-button" onClick={() => toggleSort("client")}>Cliente <span>{sortIndicator("client")}</span></button></th><th><button className="table-sort-button" onClick={() => toggleSort("status")}>Status / Etapa <span>{sortIndicator("status")}</span></button></th></tr></thead><tbody>{visibleOrders.map((order) => <tr key={`${order.id}-${order.work}`}><td><strong className="order-id">#{order.id}</strong></td><td><strong>{order.version}</strong></td><td><strong>{order.order}</strong></td><td><strong>{order.name}</strong></td><td><strong>{order.client}</strong></td><td><span className={`status-pill nucleus-status-${getNucleusStatusTone(order.status, order.isClosed)}`}><i />{order.status}</span></td></tr>)}</tbody></table>{visibleOrders.length === 0 && <div className="empty-state"><strong>Nenhuma ordem encontrada</strong><span>Ajuste os filtros ou sincronize os dados na dashboard.</span></div>}</div>}
